"use client";
import { deleteTechnician, revalidateClientPath } from "@/lib/actions";
...
const DeleteTechnician = ({ technician }: { technician: ITechnician }) => {
  const { isOpen, onOpen, onOpenChange } = useDisclosure();

  const handleClickDelete = async () => {
    const createMsg = toast.loading("Cargando...");
    try {
      const response = await deleteTechnician(technician.id);
      if (!response?.id) throw response;
      onOpenChange();
      revalidateClientPath("/dashboard/technicians");
      toast.success("El técnico se eliminó correctamente");
    } catch (error) {
          ...
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col text-center">
                Estas apunto de eliminar el técnico {technician.name}
